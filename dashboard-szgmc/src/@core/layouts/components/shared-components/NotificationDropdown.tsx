...
const NotificationDropdown = (props: Props) => {
  ...
  const mappedNotifications = notifications?.map(item => ({
    ...item,
    title: isAR ? item?.title_ar : item?.title,
    meta: isAR ? item?.meta_ar : item?.meta
  }))
    ...
    <Fragment>
      <IconButton color='inherit' aria-haspopup='true' onClick={handleDropdownOpen} aria-controls='customized-menu'>
        <Badge
          color='error'
          variant='dot'
          invisible={!mappedNotifications.length}
          sx={{
        ...
        >
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%' }}>
            ...
            <CustomChip
              ...
              size='small'
              color='primary'
              label={`${mappedNotifications.length} New`}
              sx={{ fontSize: '0.75rem', fontWeight: 500 }}
            />
          </Box>
        </MenuItem>
        <ScrollWrapper hidden={hidden}>
          {mappedNotifications.map((notification: NotificationsType, index: number) => (
